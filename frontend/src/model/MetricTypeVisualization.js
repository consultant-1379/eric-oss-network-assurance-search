...
class MetricTypeVisualization {
  ...
  static constructFromObject(data, obj) {
    if (data) {
      ...
      if (Object.prototype.hasOwnProperty.call(data, 'groups')) {
        obj.groups = ApiClient.convertToType(data.groups, { String: ['String'] });
      }
    }
    return obj;
  }
  ...
  static validateJSON(data) {
    ...
    if (!Array.isArray(data.category)) {
      throw new Error(
        `Expected the field \`category\` to be an array in the JSON data but got ${data.category}`,
      );
    }
    // ensure the json data is a string
    if (!MetricTypeVisualization.jsonStringValidation(data, 'query')) {
      throw new Error(
        `Expected the field \`query\` to be a primitive type in the JSON string but got ${data.query}`,
      );
    }
